...
int pwmPin_hand = 6;   // PWN output pin for handle motor -- AY: NEWLY ADDED
int dirPin_hand = 7;   // direction output pin for handle motor -- AY: NEWLY ADDED
int sensorPosPin = A2; // input pin for MR sensor
int ERMpwmPin = 3;     // PWM output pin for ERM motor -- AY: CHANGED TO PIN 3
int ERMdir = 12;        // direction output pin for ERM motor

const int ENC1_A = 2;  // Encoder channel A pin -- AY: NEWLY ADDED
const int ENC1_B = 4;  // Endoder channel B pin -- AY: NEWLY ADDED
...
double dxh_filt_prev2;   // AY: UNUSED?
...
// double rh = 0.052;  //[m] AY: SHOULD BE THIS, ONLY USING 0.0875 FOR TEST PURPOSES
double rh = 0.0875;
// double rp = 0.007;  //[m] 7.0mm AY: <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
// double rs = rh;     //[m] 5.2cm AY: <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
void setup() 
{
  ...
  pinMode(pwmPin_hand, OUTPUT);  // PWM pin for handle motor -- AY: NEWLY ADDED
  pinMode(dirPin_hand, OUTPUT);  // dir pin for handle motor -- AY: NEWLY ADDED
  ...
  analogWrite(pwmPin_hand, 0);     // set to not be spinning (0/255) - AY: NEWLY ADDED
  digitalWrite(dirPin_hand, LOW);  // set direction - AY: NEWLY ADDED
...
void loop()
{
  ...
  // Damping coefficient
  double b = 8;        // increase from 5
  double k_base = 120;    // reduce from 80 if overshoot is strong

  // Center is zero
  double xh_error = xh;

  dxh_filt = 0.05 * dxh + 0.95 * dxh_filt_prev; // filter the noise <<<<< AY: not sure if we need this, already done above

  double position_deadband = 0.003;
  double velocity_deadband = 0.03;

  // Spring + damping return to center
  if (abs(xh_error) < position_deadband && abs(dxh_filt) < velocity_deadband) {
      force_base = 0;
  } else {
      force_base = -k_base * xh_error - b * dxh_filt;
  }

  Tp_base = force_base * rp / rs;
  dxh_filt_prev = dxh_filt; // <<<<<<<<<<<<<<<<<<<<<< AY: also not sure if we need this if we don't have line 214
  ...
  // Serial.print(" theta_hand_deg=");
  // Serial.print(theta_hand_deg, 3);

  // Serial.print(" xh_hand=");
  // Serial.println(xh_hand, 6);
  ...
  double kw = 300; // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< AY: SETTING THIS LOW FOR SAFETY!!
  double wall_position = 0.0005; // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< AY: CAN EXPERIMENT WITH!!
  ...
  // Contact detection for BOTH directions
  bool currentlyInContact = (abs(xh_hand) > wall_position);
  ...
  // Virtual wall force
  if (currentlyInContact) {
    // Virtual wall / impact force
    force_hand = -kw * xh_hand;
  } else {
    // Return handle back to center
    double k_center = 80;
    double b_center = 2;
    double center_deadband = 0.0001;

    // Center is zero
    double xh_hand_error = xh_hand;

    if (abs(xh_hand_error) > center_deadband) {
        force_hand = -k_center * xh_hand_error - b_center * dxh_hand_filt;
    } else {
        force_hand = 0;
    }
  }
...
// Minimum PWM only when touching wall and return to center
if (output_hand > 0 && output_hand < 40) {
  output_hand = 40;
}
...
Serial.print(" xh_hand=");
Serial.print(xh_hand, 6);

Serial.print(" wall=");
Serial.print(wall_position, 6);

Serial.print(" contact=");
Serial.print(currentlyInContact);

Serial.print(" force_hand=");
Serial.print(force_hand, 5);

Serial.print(" output_hand=");
Serial.println(output_hand);
